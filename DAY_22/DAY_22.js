// ********JavaScriptmas_DAY-22_Challenge**********
...
function displayTables() {
   // getReadyTables() return a array on which we are mapping over and returning an html for each item in the array.
   // Also joining it with nothing so that we don't get the commas in between.
   return getReadyTables()
      .map((table) => `<div class="table">${table}</div>`)
      .join("");
   //For more understanding.
   // let readyTables = getReadyTables();
   // return readyTables.map( table => `<div class="table">${table}</div>` ).join("");
}
